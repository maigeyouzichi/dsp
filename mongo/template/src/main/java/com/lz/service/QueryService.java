...
public class QueryService {
    ...
    public List<User> findByCondition() {
        ...
        String sex = "男";
        // 创建条件对象
        Criteria criteria = Criteria.where("sex").is(sex).and("age").lt(20);
        // 创建查询对象，然后将条件对象添加到其中
        Query query = new Query(criteria);
        // 查询并返回结果
     ...
     * @return 符合条件的文档列表
     */
    public List<User> findByConditionAndSort() {
        // 设置查询条件参数
        String sex = "男";
        String sort = "age";
        // 创建条件对象
        ...
        Query query = new Query(criteria).with(Sort.by(sort).descending());
        // 执行查询
        List<User> documentList = mongoTemplate.find(query, User.class, COLLECTION_NAME);
        // 输出结果
        for (User user : documentList) {
